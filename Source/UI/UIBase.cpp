...
void UIBase::__LoadScene(void)
{
	for (size_t i = 0; i < m_UISceneList.size(); i++)
	{
		...
		{
			//I3PRINTLOG(I3LOG_FATAL, "Load UIScene %s", pInfo->_szPath);
			i3_prof_scope("__DirectlyLoadScene call");
			__DirectlyLoadScene(pInfo);
		}
		else
		{
			//I3PRINTLOG(I3LOG_FATAL, "Re-Bind UIScene %s", pInfo->_szPath);
...
/*virtual*/ i3UIScene* UIBase::_LoadScene(const char* pszScenePath, LuaFuncReg* GlueFunc, bool bSceneEnable)
{
	...
	//I3PRINTLOG(I3LOG_FATAL, "Load UIScene %s", pszScenePath);
	...
	if (pUIScene != nullptr)
	{
		...
		//I3PRINTLOG(I3LOG_FATAL, "UIScene %s loaded", pszScenePath);
		i3_prof_start("__BindScene call");
		__BindScene(pUIScene, GlueFunc, bSceneEnable);
		i3_prof_end();
	}
	else if (pUIScene == nullptr) {
		I3PRINTLOG(I3LOG_FATAL, "Could not load UIScene file. %s", pszScenePath);
		pUIScene = nullptr;
	}
...
void UIBase::__BindScene(i3UIScene* pScene, LuaFuncReg* GlueFunc, bool bEnable)
{
	...
	//I3PRINTLOG(I3LOG_FATAL, "Bind UIScene %s", pScene->GetName());
...
{
	I3ASSERT(pScene != nullptr);
	//I3PRINTLOG(I3LOG_FATAL, "Unbind UIScene %s", pScene->GetName());
...
i3UIScene* UIBase::AddScene(const char* pszScenePath, const LuaFuncReg* GlueFunc /*= nullptr*/, bool bLoaded /*= false*/, bool bSceneEnable /*=true*/)
{
	...
	//I3PRINTLOG(I3LOG_FATAL, "[AddScene] Cargando escena: '%s'", strFullPath.c_str());

	// ✅ VERIFICAR QUE EL ARCHIVO EXISTE ANTES DE INTENTAR CARGARLO
	if (!i3FileStream::isFile(strFullPath.c_str()))
	{
		I3PRINTLOG(I3LOG_FATAL, "[AddScene] Archivo de escena no encontrado: '%s'", strFullPath.c_str());
		//I3PRINTLOG(I3LOG_FATAL, "[AddScene] Carpeta UI: '%s'", strCommonUIFolder.c_str());
		//I3PRINTLOG(I3LOG_FATAL, "[AddScene] Ruta relativa: '%s'", pszScenePath);

		// Intentar cargar una escena de respaldo o crear una vacía
		return nullptr; // Por ahora retornamos nullptr para evitar el crash
	}
	...
	if (bLoaded)
	{
		...
		// ✅ VERIFICACIÓN MEJORADA DE FALLO DE CARGA
		if (pInfo->_pScene == nullptr)
		{
			I3PRINTLOG(I3LOG_FATAL, "[AddScene] Error: No se pudo cargar la escena '%s'", strFullPath.c_str());
			I3PRINTLOG(I3LOG_FATAL, "[AddScene] Verifica que el archivo existe y es válido");

			// Limpiamos la entrada fallida del registro de escenas
			i3::vu::remove(m_UISceneList, pInfo);
			I3_SAFE_DELETE(pInfo);
